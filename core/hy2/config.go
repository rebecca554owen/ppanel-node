...
func (n *Hysteria2node) getConn(info *panel.NodeInfo, config *conf.Options) (net.PacketConn, error) {
	uAddr, err := net.ResolveUDPAddr("udp", formatAddress(config.ListenIP, info.Common.Hysteria2.Port))
	if err != nil {
		return nil, err
	}
	...
	switch strings.ToLower(info.Common.Hysteria2.ObfsType) {
	case "", "plain":
		return conn, nil
	case "salamander":
		ob, err := obfs.NewSalamanderObfuscator([]byte(info.Common.Hysteria2.ObfsPassword))
		if err != nil {
			return nil, err
		}
...
func (n *Hysteria2node) getBandwidthConfig(info *panel.NodeInfo) *server.BandwidthConfig {
	...
	if info.Common.Hysteria2.UpMbps != 0 {
		band.MaxTx = (uint64)(info.Common.Hysteria2.UpMbps * Megabyte / 8)
	}
	if info.Common.Hysteria2.DownMbps != 0 {
		band.MaxRx = (uint64)(info.Common.Hysteria2.DownMbps * Megabyte / 8)
